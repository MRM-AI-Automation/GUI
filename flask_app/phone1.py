...
def handle_capture_image(image_data_url):
    # Save or process the captured image data
    save_to_file('captured_image.txt', image_data_url)
    print('Captured Image:', image_data_url)

@socketio.on('capture_sensor_values')
def handle_capture_sensor_values(sensor_data):
    # Save or process the captured sensor data
    save_to_file('captured_sensor_values.txt', json.dumps(sensor_data))
    print('Captured IMU Data:', sensor_data['imu'])
    print('Captured GPS Data:', sensor_data['gps'])
...
def udp_server():
    ...
    server_socket.bind(('192.168.15.172', 7777))
